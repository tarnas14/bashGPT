...
get_answer () {
    ...
    local previousQueries=(${@:2})

    local allQueries=($previousQueries)
    allQueries+=($query)
    ...
    local chatGptMessages=()
    for message in "${allQueries[@]}"
    do
        chatGptMessages=(${chatGptMessages[@]} "{\"role\": \"user\", \"content\": \"$message\" }")
    done

    local messagesJson=$(IFS=,; echo "${chatGptMessages[*]}")

    local result=$(curl --location --request POST 'https://api.openai.com/v1/chat/completions' \
    ...
    local resultPreparedToDisplayInRofi=$(echo $result | fold -w90 | awk 'NF')
    local nextQuery=$(echo $resultPreparedToDisplayInRofi | rofi_prompt)
    ...
    get_answer $nextQuery $allQueries
}

firstQuery=$(echo "" | rofi_prompt)
get_answer $firstQuery
